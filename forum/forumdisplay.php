	...
	<div id="ct" class="wp cl{if $_G['forum']['allowside']} ct2{/if}"{if $leftside} style="margin-left:{$_G['leftsidewidth_mwidth']}px"{/if}>
		...
		<div class="mn">
			<!--{if $_G['page'] == 1 && $_G['forum']['rules']}-->
				...
				<div class="ggq-sbon ggq-bkgz">
					<div class="bm_h cl">
						<span class="o"><img id="forum_rules_{$_G[fid]}_img" src="{IMGDIR}/collapsed_{$collapse[forum_rulesimg]}.gif" title="{lang spread}" alt="{lang spread}" onclick="toggle_collapse('forum_rules_{$_G[fid]}')" /></span>
						<h2>版块规则</h2>
					</div>
					<div id="forum_rules_{$_G[fid]}" class="bkgz-gzon">
						$_G['forum']['rules']
					</div>
				</div>
			<!--{/if}-->
			...
			<!--{if !$subforumonly}-->
				...
				<div id="pgt" class="bm bw0 pgs ggq-zfan cl">
					<!--{if !$_GET['archiveid']}--><a href="javascript:;" id="newspecial" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="{lang send_posts}">发布主题</a><!--{/if}-->
		...
		<div class="sd">
			...
			<div class="ggq-bkon cl">
				<div class="bkon-bg{if $_G[forum][banner] && !$subforumonly}" style="background: url($_G[forum][banner]) no-repeat;background-size: cover;background-position: center;"{else} bkon-bgy"{/if}>
					<div class="bkbg-zz"></div>
				</div>
				<!--{if $_G[forum][rank]}--><div class="bkon-bkpm" title="{lang previous_rank}:$_G[forum][oldrank]">TOP.$_G[forum][rank]</div><!--{/if}-->
				<div class="bkon-ico">
					<img src="{if $_G['forum'][icon]}data/attachment/common/$_G['forum'][icon]{else}template/guiigo_qing/images/groupicon.gif{/if}" alt="$_G['forum'][name]">
				</div>
				<div class="bkon-btjj">
					<a href="forum.php?mod=forumdisplay&fid=$_G[fid]" class="btjj-bt">$_G['forum'][name]</a>
					<!--{if (!empty($_G[forum][domain]) && !empty($_G['setting']['domain']['root']['forum'])) || $moderatedby }-->
					<div class="bkon-ymbz cl">
						<!--{if !empty($_G[forum][domain]) && !empty($_G['setting']['domain']['root']['forum'])}-->
							<div class="ymbz-li ymbz-ym">{lang forum_domain}: <a href="{$_G['scheme']}://{$_G[forum][domain]}.{$_G['setting']['domain']['root']['forum']}" id="group_link">{$_G['scheme']}://{$_G[forum][domain]}.{$_G['setting']['domain']['root']['forum']}</a></div>
						<!--{/if}-->
						<!--{if $moderatedby}-->
							<div class="ymbz-li ymbz-bz">{lang forum_modedby}: $moderatedby</div>
						<!--{/if}-->
					</div>
					<!--{/if}-->
					<p><!--{if $_G['forum'][description]}--><!--{echo cutstr($_G['forum'][description],85)}--><!--{else}-->管理员还未对该版块设置版块简介<!--{/if}--></p>
				</div>
				<div class="bkon-czan cl">
					<a href="home.php?mod=spacecp&ac=favorite&type=forum&id=$_G[fid]&handlekey=favoriteforum&formhash={FORMHASH}" id="a_favorite" class="czan-sc z" onclick="showWindow(this.id, this.href, 'get', 0);">+ 收藏</a>
					...
					<div id="ggq-bkc_menu" class="czan-on" style="display: none;">
						<!--{if rssforumperm($_G['forum']) && $_G[setting][rssstatus] && !$_GET['archiveid'] && !$subforumonly}--><a href="forum.php?mod=rss&fid=$_G[fid]&auth=$rssauth" target="_blank" title="RSS">RSS{lang rss_subscribe_this}</a><!--{/if}-->
				...
				<div class="bkon-bksj cl">
					<ul>
						<li>
							...
							<div class="bksj-sj">$_G[forum][todayposts]</div>
						</li>
						<li>
							<div class="bksj-bt">主题</div>
							<div class="bksj-sj">$_G[forum][threads]</div>
						</li>
						<li>
							<div class="bksj-bt">收藏</div>
							<div id="number_favorite_num" class="bksj-sj">$_G[forum][favtimes]</div>
						</li>
					</ul>
				</div>
			</div>
