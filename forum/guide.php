...
<div class="boardnav">
	<div id="ct" class="wp cl{if $_G['forum']['allowside']} ct2{/if}"{if $leftside} style="margin-left:{$_G['leftsidewidth_mwidth']}px"{/if}>
		<div class="mn">
			...
			<!--{if $view != 'index'}-->
			<div id="pgt" class="bm bw0 pgs ggq-fbzt cl">
				$multipage
				<a href="forum.php?mod=misc&amp;action=nav" class="fbzt-ddfb" onclick="showWindow('nav', this.href, 'get', 0)">发布主题</a>
			</div>
			<!--{/if}-->
			...
			<!--{else}-->
				<!--{loop $data $key $list}-->
				<div id="threadlist" class="tl bm bmw ggq-ddon"{if $_G['uid']} style="position: relative;"{/if}>
					<div class="th">
